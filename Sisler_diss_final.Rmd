...
## M note:THIS CHUNK IS EXPECTING RDATA!
...
#IMPORT file
 ...
 data.raw <- read.csv("./data/Sisler_dissertation_HMS1819_audit.csv")
...
data.v3 <- data.v3 %>%  
  mutate(gender_tgd = case_when(gender == 2 ~ 1,
                                ...
                                gender == 1 ~ 0)) %>%
  filter(!is.na(gender))
data.v3$gender_tgd <- as.numeric(data.v3$gender_tgd)
table(data.v3$gender_tgd, useNA = "always")
```

### Reconcile 'gender_text' 
## M note: editing here, starting w 95 lines, ending w 50
```{r gender_text}
  # gender-diverse

#transgender (1)
trans_m <- c("Demi","Binary","Neutral","Queer","Fluid","Agender","Androgynous","Pangender","Bigender","demi","binary","neutral","queer","fluid","agender","androgynous","pangender","bigender","she/they", "Closeted genderfluid (female presenting)", "Pan-gender","female / infinite star being / gender non conforming","He or they","Not sure, nonbinary I guess", "Non-binary trans maculine", "No preference", "Somewhere between GNC and transfemme", "She/they","Non-binary leaning towards trans man", "Transmasculine", "Trans-masculine person", "Trans Masculine", 
"gender non-conforming trans man", "I'm not yet sure where I identify", "Questioning","Unsure","Idk","questioning","unsure","idk","Hermaphrodite","Questioning","Undecided","Literally no clue as of late", "Kynigender", "Adrogynous", "Hermaphrodite", "idk yet", "Questioning", "Questioning; female", "Still questioning", "Not sure","not female/questioning", "Questioning (unsure)", "Girl ? Idk", "not sure", "Not Sure", "I don't know, still trying to purse it out", "Undecided")

#cisgender (0)
cis_m <- c("Dude","Male","Boy","dude","male","boy","Male: there are only 2 genders", "if you have a penis you're a male", "Cisgender man", "Male motherfucker. stop making up gender shit.", "He-male","I have male parts so therefore I am a MALE and there are only TWO genders!!!!!!!!!!!!!!!!!","SATSYG", "My sex is male", "I say male, but I think gender is fluid", "Theres only two genders, I'm male", "Probably heterosexual, but so commitment-phobic that I can't actually tell if I'm asexual.","I'm a male. Born a male, always will be a male. There should only be two choices.","Female","Woman","Girl","female","woman","girl","Male and Female are not genders, they are sexes. I identify as a woman.", "Female, but currently going through a gender identity crisis!")

#non-answers and memers (4)
    all_genders_m <- c(trans_m, cis_m) 

data.v3 <- data.v3 %>%
  filter(!is.na(gender_tgd)) 

#assign values to gender categories
gender_reader <- function(gender_text){
    for (i in all_genders_m) {
        if (str_detect(gender_text, i)) {
           if(i %in% trans_m){
               return(1)
           }else if(i %in% cis_m){
               return(0)
...
handbag <- data.v3 %>% 
  select(gender_text, gender_tgd) %>% 
  filter(gender_text != "")
names(handbag)[2] <- "gender_tgd_m"

for (m in 1:nrow(handbag)){
  handbag$gender_tgd_m[m] <- gender_reader(handbag$gender_text[m])
}

data.v3 <- left_join(data.v3, unique(handbag), by = "gender_text")

for (m in 1:nrow(data.v3)) {
  if (data.v3$gender_text[m] != ""){
    data.v3$gender_tgd[m] <- data.v3$gender_tgd_m[m]
  }
}
...
data.v4$sexual <- as.numeric(data.v4$sexual)
data.v4 <- data.v4 %>%
  filter(!is.na(sexual)) 
...
## M note: working here, starting w 130 lines, ending w
```{r sexual_text}
# identifying straight individuals:
straight_m <- c("Hetero","Straight","Hetrosex","hetero","straight","hetrosex","Kinsey scale 1", "I would put myself as a 1-1.5 on the kinsey scale", "Staight", "Stragiht", "strait", "Str8!!!!", "Changing from gay to heterosexual","Gynophilic", "Into females", "I fuck bitches.", "1 on the Kinsey Scale","Gynophilic", "gynosexual", "Changing from gay to heterosexual", "100% Hetro")


# identifying queer  individuals:
queer_m <- c("[Q|q]ueer","[F|f]luid","[O|o]pen","potentially interested", "somewhere between", "homoflexible", "heteroflexible","fluid", "Only other trans people", "non-binary", "so I guess I could check queer","Androsexual", "Not straight", "Polysexual", "Joto", "Non-binary", "nonbinary","Attracted to masculinity and androgony", "Androsexual","[Q|q]uestion","[P|p]robably","possibly", "curious", "Unknown", "potentially interested", "don't know", "Unknown","don't know", "not sure", "still figuring it out", "idk", "Idk",  "don't know", "I have a girlfriend but i am not attracted to any other females. I am attracted to males", "Sometimes gender doesn't matter and I don't have time to learn about these things life gets too busy","Not sure. Working on it.","[P|p]an","[S|s]apiosexual","all", "Polyamorous Pansexual", "Pan Sexual", "Pan-sexual", "any but ive only had relationships with men","[G|g]r[a|e]y","[N|n]onsexual","[C|c]elibate","[D|d]emi","[A|a]sex","[C|c]elibate","[A|a]ce","Mostly asexual", "Most likely asexual", "Masexual", "Tridium Vykon Jace 8000","Demi-romantic", "asexual lesbian", "Ase", "Bi-demisexual", "A sexual","Hetroromantic Asexual", "Demiqueer quasiflux asexual foxkin", "DemiAsexual","a-sexual", "no sexual attraction", "a sexual", "Homoromantic Demisexual","Gay-ish, I am Demi-sexual which means I don't pay attention to sex/gender", "experience no sexual attraction but do experience a small amount of romantic attraction", "Pan-Quoiromantic Asexual", "demiromantic asexual","[G|g]ay","[H|h]omoflexible","Mostly gay", "soooooo gay", "Romantically straight and sexually gay", "[N|n]ot [S|s]ure","[L|l]esbian","[D|d]yke","[B|b]isex","[B|b]icurious","I'm straight but I like girls too sexually, not emotionally.", "Half bisexual","straight but DTF sexy females", "bicurious but i like girls more","hetero with occasional bi tendencies", "gay leaning bi", "Bi-curious","Herero-romantic bisexual", "Bi with a stronger attraction to same sex","I'm like halfway to bisexual. Never had a crush on another girl but I'm still interested. Idk.","No boundaries in place - though probably bisexual")

#identify memers and others
all_sexualities_m <- c(straight_m,queer_m)

#assign values to sexuality categories
sex_reader <- function(sexuality_text){
    for (i in all_sexualities_m) {
        if (str_detect(sexuality_text, i)) {
           if(i %in% queer_m){
               return(1)
           }else if(i %in% straight_m){
               return(0)
           } 
         }
      } 
...
suitcase <- data.v4 %>% 
  select(sexual_text, sexual) %>% 
  filter(sexual_text != "") 
names(suitcase)[2] <- "sexual_m"

#apply reader
for (m in 1:nrow(suitcase)){
  suitcase$sexual_m[m] <- sex_reader(suitcase$sexual_text[m])
}

#reconcile reader data
data.v4 <- left_join(data.v4, unique(suitcase), by = "sexual_text")

for (m in 1:nrow(data.v4)) {
  if (data.v4$sexual_text[m] != ""){
    data.v4$sexual[m] <- data.v4$sexual_m[m]
  }
}
...
data.v5 <- data.v4 %>%   # fix directionality
...
```
## M note: is 60 hrs nonsense? also, is tabulating in final?
...
## M note: starting here w 195 lines in chunk, maintaining categories as-is, only streamlining code
## M note: why are Americans, Humans, Mixed Race, and Middle East all #5?
## M note: Race categories fucky -> Armenian in both 1 and 5, mixed race in 1 and 5, atlantic islander in 4, Bengladesh misspelled, Kekistan in Middle east, Canad is weird, Wakanda in 4,

```{r race reconciliation}
##Define race categories
#White 
  white_m <- c("Jew", "jew", "Ashkenazi","ashkenazi","Caucasian","caucasian","European","european","Greek","greek","Irish","irish","Italian","italian","Albania","albania","Semetic","semetic","Semitic","semitic","Russian","russian","Roma","roma","Turkish","turkish","Slavic","slavic","Metis","metis","Basque","basque","Sicilian","sicilian","Polish","polish","Scandinavian","scandinavian","Finnish","finnish","Celtic","celtic","Scottish","scottish", "Canad","canad", "Mediterranean", "mediterranean", "please refer to me as Caucasian; it's extremely offensive to refer to every race-- excluding Caucasians-- in their politically correct form","Not sure if I'm Hispanic, as I am Portuguese and Spanish (from my mother's side) . I identify as white as I am of European descent.","Portugese", "Cape Verdean", "Capeverdean", "Bulgarian-American", "Ginger", "Anglo Saxon", "anglo/white", "Balkan for ethnicity", "Ulster-scots", "I'm white, but I do have some Native American in me but it's not the dominant visible ethnicity.", "White (Jewish/Jamaican-Chinese/Irish)", "ARMENIAN")

#Black
  black_m <- c("African","Afro","Kenya","Black","Carribean","Bahamian","Haitian","Jamaica","Zimbabwea","Ghana","Ethiopia","Trinidad","Eritrean","Wakanda","Nigerian","african","afro","kenya","black","carribean","bahamian","haitian","jamaica","zimbabwea","ghana","ethiopia","trinidad","eritrean","wakanda","nigerian","Cameroonian American", "Creole", "Jamacian American", "North Africa", "North african", "North African", "north african")

#Hispanic
  hispanic_m <- c("Mexican","Xicano","Xicana","Latino","Latina","Puerto Rican","Puerto rican", "Spanish", "Chicano", "Chicana", "Iberian","Mestizo","Mestiza","Portuguese","Brazil","Cuba","mexican","xicano","xicana","latino","latina","puerto Rican", "puerto rican","spanish", "chicano","chicana", "iberian","mestizo","mestiza","portuguese","brazil","cuba","Costa Rican", "Central American Native", "Andean")

#Asian
  asian_m <- c("Asian","Chinese","Japanese","Indian","Banglades","Filipino","Guyanese","Mongolia","China","Southeast","South Asian","South asian","Punjabi","Siberia","Indonesia","West |ndian","West indian", "asian","chinese","japanese","indian","banglades","filipino","guyanese","mongolia", "china","southeast","south Asian","south asian","punjabi","siberia","indonesia","west Indian","west indian", "Sri Lankan", "Sri-Lankan", "Malays", "Chinese Ethinicy-Nuosu", "Austronesian","third culture kid, born and raised in asia until 18yo", "Central Asian","Indian, UAE native", "Indian/Subcontinental", "Indians arent on the list.....", "American Texan of Indian (South asia) Descent", "Okinawan", "Asian/Chinese","Japanese American and Native Hawaiian Culture", "Asian. International.", "Australian/Indian","Indo-Caribbean",  "Atlantic Islander", "Telugu-speaking Tamil-ethnicity Indian")

  #Other not these
    all_races_m <- c(white_m, black_m, hispanic_m, asian_m) 

    
#assign values to race categories
race_reader <- function(race_text){
    for (i in all_races_m) {
        if (str_detect(race_text, i)) {
           if(i %in% white_m){
               return(1)
           } else if(i %in% black_m){
               return(2)
           } else if(i %in% hispanic_m) {
               return(3)
           } else if(i %in% asian_m) {
               return(4)
           }
        }
      } 
...
purse <- data.v6 %>% filter(!is.na(race_other_text), race_other_text != "") %>% 
  select(race_other_text, race) 
names(purse)[2] <- "race_m"

for (m in seq(1,nrow(purse),1)){
  purse$race_m[m] <- race_reader(purse$race_other_text[m])
}

data.v6 <- left_join(data.v6, unique(purse), by = "race_other_text")

for (m in 1:nrow(data.v6)) {
  if (!is.na(data.v6$race_m[m])){
    data.v6$race[m] <- data.v6$race_m[m]
  }
}
